...
/* CONTENT */
.content { text-align:justify; margin-bottom:5px; }
.section { margin-bottom:5px; }
.section p { margin:1px 0; }
.section .label { display:inline-block; width:150px; font-weight:bold; }

/* SIGNATURE */
.signature { text-align:right; margin-top:15px; }
.signature .space { margin-top:25px; margin-bottom:3px; }

/* OPTIONAL: prevent page break inside section */
...
<p><span class="label">Tempat/Tgl Lahir:</span> {{ $data['tempat_lahir'] ?? '-' }}, @if(isset($data['tanggal_lahir'])) {{ \Carbon\Carbon::parse($data['tanggal_lahir'])->translatedFormat('d F Y') }} @else - @endif</p>
...
<p><span class="label">Tanggal Kematian:</span> {{ isset($data['tanggal_kematian']) ? \Carbon\Carbon::parse($data['tanggal_kematian'])->translatedFormat('d F Y') : '-' }}</p>
